...
public class EventDAO
{
    ...
    private static string connectionString = ConfigurationManager.ConnectionStrings["RelianceConnectionString"].ConnectionString.ToString();
    ...
    public static Int32 addEvent(CalendarEvent cevent)
    {
        //add event to the database and return the primary key of the added event row
      
        //insert
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("INSERT INTO Tbl_Calendar(title, description, event_start, event_end) VALUES(@title, @description, @event_start, @event_end)", con);
